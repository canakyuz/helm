...
export const LoginPage = () => {
  ...
  const submit = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(normalizedEmail) || password.length < 6) {
      setValidationError("Geçerli bir e-posta ve en az 6 karakter şifre gir.");
      return;
    }
    setValidationError("");
    reset();
  ...
  const formError = validationError || (error ? "E-posta veya şifre hatalı. Tekrar dene." : "");
    ...
    <main className="min-h-screen bg-bento-canvas text-bento-fg">
      <div className="mx-auto flex min-h-screen w-full max-w-6xl flex-col px-5 py-6 sm:px-8 lg:px-12">
        <header className="flex items-center justify-between">
          ...
          <span className="font-mono text-[10px] uppercase tracking-[0.18em] text-bento-fg3">Founder cockpit</span>
        </header>
        ...
        <div className="grid flex-1 items-center gap-12 py-14 md:grid-cols-[1fr_420px] md:gap-20 lg:py-20">
          <section className="hidden max-w-xl md:block" aria-labelledby="welcome-title">
            <div className="mb-8 flex items-center gap-3 font-mono text-[10px] uppercase tracking-[0.18em] text-bento-fg3">
              ...
              Control room / secure access
            </div>
            <h1 id="welcome-title" className="max-w-lg text-5xl font-semibold leading-[1.02] tracking-[-0.04em] lg:text-6xl">Geliri tek bakışta yönet.</h1>
            <p className="mt-6 max-w-md text-base leading-7 text-bento-fg2">Projelerini, kaynaklarını ve günlük hareketi aynı kokpitte takip et. Helm, karar vermen gereken sayıyı öne çıkarır.</p>
            <div className="mt-12 flex items-center gap-8 border-t border-bento-line pt-5 font-mono text-[11px] uppercase tracking-[0.12em] text-bento-fg3">
              <span className="inline-flex items-center gap-2"><ShieldCheck className="size-4 text-bento-pos" /> Private workspace</span>
              <span className="inline-flex items-center gap-2"><LockKeyhole className="size-4 text-bento-blue" /> Supabase auth</span>
            </div>
          </section>
          ...
          <section className="w-full" aria-labelledby="login-title">
            <div className="rounded-[22px] bg-bento-tile p-6 ring-1 ring-bento-line sm:p-8">
              <div className="mb-8">
                <p className="font-mono text-[10px] uppercase tracking-[0.18em] text-bento-fg3">Secure sign in</p>
                <h2 id="login-title" className="mt-3 text-2xl font-semibold tracking-tight">Tekrar hoş geldin.</h2>
                <p className="mt-2 text-sm leading-6 text-bento-fg2">Kokpitine devam etmek için bilgilerini gir.</p>
              </div>
              ...
              <form onSubmit={submit} className="space-y-5" noValidate>
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="font-mono text-[10px] uppercase tracking-[0.12em] text-bento-fg3">Min. 6 karakter</span>
                  </div>
            ...
            <p className="mt-5 text-center font-mono text-[10px] uppercase tracking-[0.14em] text-bento-fg3">helm / private workspace</p>
          </section>
        </div>
      </div>
    </main>
