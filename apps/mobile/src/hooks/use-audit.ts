...
async function fetchAudit(propertyId: SelectedPropertyId): Promise<AuditEntry[]> {
  // FK kolon adı `project_id` (rename öncesi convention korundu), `properties` tablosuna referans.
  let q = supabase
    .from("audit_log")
...
export function useAudit() {
  ...
  return useQuery({
    queryKey: ["audit", "recent", selectedPropertyId],
    queryFn: () => fetchAudit(selectedPropertyId),
    staleTime: 5 * 60_000,
  });
}
