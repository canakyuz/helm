...
// Heartbeat sağlık eşiği:
//  - last_ping_at < interval  → healthy
//  - interval ≤ diff < 2× interval → stale
//  - diff ≥ 2× interval  → down
//  - hiç ping yok → unknown
...
async function fetchProperties(): Promise<Property[]> {
  ...
  return ((data as unknown as PropertyRow[] | null) ?? []).map((row) => {
    ...
    return {
      ...
      status: deriveStatus(latest?.last_ping_at ?? null, latest?.interval_minutes ?? null),
    };
  });
}
